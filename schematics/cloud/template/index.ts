...
import { UsersManagementGenerator } from "@janush-schematics/cloud/template/generators/usersManagement.generator";
import {
  isApiModule,
  isAuthenticationModule,
  isEmptyModules,
  isUsersManagementModule,
} from "@janush-schematics/cloud/template/validators/moduleValidators";
...
export const cloudTemplateGenerator = (options: Schema): Rule => {
  return async (tree: Tree, _context: SchematicContext) => {
    ...
    const usersManagementGenerator = new UsersManagementGenerator(options);
    ...
    return chain([
      ...
      isApiModule(options) ? schematic(CloudSchematic.API, options) : noop(),
      isUsersManagementModule(options) ? usersManagementGenerator.generate : noop(),
